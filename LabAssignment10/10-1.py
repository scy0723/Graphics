...
def drawVelocity():
    glColor3ub(255, 0, 0)
    global current, point, before
    glBegin(GL_LINES)
    glVertex3fv(before[:-1])
...
def render(t):
    ...
    R1 = np.identity(4)
    euler1 = np.array([0.,1.,0.])*t   # in XYZ Euler angles
    ...
    euler2 = np.array([1.,0.,1.])*t   # in XYZ Euler angles
    ...
    before=point
    point=J2@np.array([1,0,0,1])
    current=10*(point-before)+point
    drawVelocity()
